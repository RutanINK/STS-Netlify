...
function renderBoard() {
  ...
  const MAX_HRS = 9; // bar represents 0–9h
  let lastBuilding = null;
  const parts = [];

  cells.forEach(c => {
    // Building section header
    if (bldgF === 'all' && c.building !== lastBuilding) {
      lastBuilding = c.building;
      parts.push(`<div class="board-section-header">${esc(c.building)}</div>`);
    }

    const hrsNum = c.mins / 60;
    const hrs    = hrsNum.toFixed(1);
    const hCls   = c.mins === 0 ? 'hours-none' : hrsNum < 4 ? 'hours-green' : hrsNum < 8 ? 'hours-yellow' : 'hours-red';
    const barCls = c.mins === 0 ? 'bar-none'   : hrsNum < 4 ? 'bar-green'   : hrsNum < 8 ? 'bar-yellow'   : 'bar-red';
    const barPct = Math.min(100, (hrsNum / MAX_HRS) * 100).toFixed(1);
    ...
    // Last-saved time in EST
    const timeStr = c.submitted
      ? new Date(c.submitted).toLocaleTimeString('en-US', { timeZone:'America/New_York', hour:'numeric', minute:'2-digit', hour12:true })
      : null;

    parts.push(`<div class="board-cell-card ${cCls}" onclick="openCellDetail('${esc(c.cell)}')">
      ...
      <div class="bcc-hours-bar-wrap"><div class="bcc-hours-bar-fill ${barCls}" style="width:${barPct}%"></div></div>
      ...
      ${timeStr ? `<div class="bcc-time">⏱ ${timeStr} EST</div>` : ''}
    </div>`);
  });
...
function openCellDetail(cn) {
  ...
  const isDownNow = machineDownCells.has(cn);
  const downBtn = document.querySelector('#overlay-cell .ovl-actions button[onclick="openMachineDown()"]');
  if (downBtn) {
    downBtn.textContent = isDownNow ? 'Machine Clear' : 'Machine Down!';
    downBtn.className = isDownNow ? 'btn btn-success btn-sm' : 'btn btn-down btn-sm';
  }
...
function openMachineDown() {
  ...
  if (cn && machineDownCells.has(cn) && typeof _openClearMdownModal === 'function') {
    _openClearMdownModal(cn);
    return;
  }
  if (typeof _openMdownModal === 'function') { _openMdownModal(cn); return; }
